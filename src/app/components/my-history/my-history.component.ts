...
export class MyHistoryComponent implements OnInit {
  ...
  constructor(private eventService: EventService, private categoryService: CategoryService, private appStateService: AppStateService, private attendanceService: AttendanceService, private router: Router, private sortDataService: HelperService) {}
  ...
  sortEvents(sortData): void {
    ...
    this.events = this.sortDataService.sort(sortData, this.events);
  ...
  filterEvents(filterData): void {
    ...
    if (filterData.endingDate == null || filterData.endingDate > currentDate) {
    ...
    this.eventService.getFilteredEvents(filterData).subscribe((result) => {
      ...
      if(this.sortData == null){
        this.events = this.sortDataService.sort(this.defaultSortData, this.events);
      }
      else{
        this.events = this.sortDataService.sort(this.sortData, this.events);
      }
    });
